...
namespace HyperCasual.Runner
{
    ...
    public class CameraManager : MonoBehaviour
    {
        ...
        public void SetCameraPositionAndOrientation(bool smoothCameraFollow)
        {
            ...
            if (smoothCameraFollow)
            {
                ...
                playerPos.y = m_Transform.position.y;
                playerPos.z = m_Transform.position.z;
                var temp = Vector3.Lerp(m_Transform.position, playerPos, lerpAmound);
            ...
            if (m_CameraFollowX)
            {
                var currentTransformPos = m_Transform.position;
                currentTransformPos.x = GetPlayerPosition().x / 1.5f;
                m_Transform.position = currentTransformPos;
                /*var currentRotation = m_Transform.rotation;
                currentRotation.x = 58f;
        ...
        {
            /*Debug.LogError("Start Game Camera called");
            DOTween.To(() => m_Offset, x => m_Offset = x, new Vector3(0, 5, 5), 1.5f).OnUpdate(delegate
            {
            });*/
        }
    }
